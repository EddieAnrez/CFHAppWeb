@extends('layouts.app')
@section('main')
<h1 class="text-white text-center mt-4 mb-8 text-6xl">Noticias y Eventos</h1>
<hr class="container mb-10" >
            ...
            <aside>
                ...
                <ul>
                    @foreach ($similares as $similar)
                        <li class="mb-4">
                            <a class="flex content-between" href="{{route('blog.show',$similar)}}">
                                @if ($similar->image)
                                <img class="w-36 h-20 object-cover object-center" src="{{Storage::url($similar->image->url)}}" alt="">
                                @else
                                <img class="w-36 h-20 object-cover object-center" src="https://cdn.pixabay.com/photo/2017/03/27/14/49/beach-2179183_1280.jpg" alt="">
                                @endif
                                <span class="ml-2 text-white text-base">{{$similar->name}}</span>
                            </a>
                        </li>
                    @endforeach
                </ul>
